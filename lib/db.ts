...
export async function ensureLatestSchema() {
    try {
        ...
        await sql`CREATE TABLE IF NOT EXISTS class_info (id INTEGER PRIMARY KEY DEFAULT 1, name TEXT NOT NULL DEFAULT '', school_name TEXT NOT NULL DEFAULT '', school_year TEXT NOT NULL DEFAULT '', teacher_name TEXT NOT NULL DEFAULT '', semesters TEXT NOT NULL DEFAULT '[]', logo TEXT, print_logo BOOLEAN DEFAULT true, authorized_students TEXT DEFAULT '[]', alert_threshold INTEGER DEFAULT 2, heatmap_thresholds TEXT DEFAULT '[1,3,5]', CONSTRAINT one_row CHECK (id = 1))`;
        ...
        await sql`CREATE TABLE IF NOT EXISTS accounts (id TEXT PRIMARY KEY, username TEXT NOT NULL UNIQUE, password TEXT NOT NULL, role TEXT NOT NULL, display_name TEXT NOT NULL, student_id TEXT, team TEXT, avatar TEXT)`;
