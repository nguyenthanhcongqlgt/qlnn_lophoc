    ...
    try {
        if (hasPostgres()) {
            ...
            const { rows } = await sql`
                SELECT id, student_id AS "studentId", date, status, note
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (hasPostgres()) {
            const { sql } = await import('@vercel/postgres');
            if (status === 'present') {
                await sql`DELETE FROM attendance WHERE student_id = ${studentId} AND date = ${date}`;
            ...
            await sql`
                ...
                VALUES (${id}, ${studentId}, ${date}, ${status}, ${note || null})
                ON CONFLICT (student_id, date)
                DO UPDATE SET status = ${status}, note = ${note || null}
            `;
            return NextResponse.json({ id, studentId, date, status, note });
        }
        ...
        const id = `ATT${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
        store.attendance[key] = { id, studentId, date, status, note: note || null };
        return NextResponse.json({ id, studentId, date, status, note });
    } catch (error) {
